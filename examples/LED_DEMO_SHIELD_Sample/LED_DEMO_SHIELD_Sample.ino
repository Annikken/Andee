...
AndeeHelper button;
AndeeHelper picButton;
AndeeHelper loveButton;
AndeeHelper flashButton;
...
void setInitialData()
{
  ...
  displaybox.setId(5);  // Each object must have a unique ID number
  displaybox.setType(DATA_OUT);  // This defines your object as a display box
  displaybox.setLocation(0, 0, FULL); 
  displaybox.setColor("FF00bfff");
  displaybox.setTitleColor("50000000");
  displaybox.setTitle("Your Message"); 
  displaybox.setData(""); 
  
  button.setId(6); // Don't forget to assign a unique ID number
  button.setType(KEYBOARD_IN); // Defines object as a button
  button.setLocation(1,0,FULL);
  button.setTitleColor("FF0000FF");
  button.setTitle("Type Message");
  // Optional:
  button.setInputMode(ACK);
  // By default this is already set to true to prevent accidental presses
  // after the first button press.
  
  picButton.setId(2); // Don't forget to assign a unique ID number
  picButton.setType(CIRCLE_BUTTON); // Defines object as a button
  picButton.setLocation(2,0,HALF);
  picButton.setTitleColor("FFffd700");
  picButton.setTitle("SMILEY");
  
  loveButton.setId(3); // Don't forget to assign a unique ID number
  loveButton.setType(CIRCLE_BUTTON); // Defines object as a button
  loveButton.setLocation(2,1,HALF);
  loveButton.setTitleColor("FF9400d3");
  loveButton.setTitle("Love");
  
  flashButton.setId(4);
  flashButton.setType(BUTTON_IN); // Defines object as a button
  flashButton.setLocation(3,0,FULL);
  flashButton.setTitleColor("FFff4500");
  flashButton.setTitle("Flash");
}
void setup() {
  
  Andee.begin();  // Setup communication between Annikken Andee and Arduino
  Andee.setName("Andee LED Demo");
...
{ 
  ...
  if( button.isPressed() )
  {
    ...
    button.ack(); 
    memset(dispString, 0x00, 32); // Empty the contents of the string before receiving user input
    button.getKeyboardMessage(dispString); // Display keyboard and store input into userInput
    //displaybox.updateData(dispString); // Display user input on the display box
    mode = 0;
  }
  
  if(picButton.isPressed()){
    picButton.ack();
    mode = 1;
  }
  
  if(loveButton.isPressed()){
    loveButton.ack();
    mode = 2;
  }
  ...
  displaybox.setData(dispString); // Don't forget to call update() or else your objects won't show up
